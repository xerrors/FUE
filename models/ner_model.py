...
class EntDecoder(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # dry_test 阶段，不计算中间过程的 hidden state
...
class NERModel(pl.LightningModule):
    ...
    def forward(self, hidden_states, labels=None, graph=None, mask=None, mode="train"):
        ...
        if self.config.use_ner_layer_loss: # default: True
            ...
            if self.config.use_first_layer_loss:  # default True
                layer_logits = out["logits_out"][:-1]
            else:
                layer_logits = out["logits_out"][1:-1]

            for lo_i, logits_out in enumerate(layer_logits[::-1]):
    ...
    def decode_entities(self, logits, pos=None, with_score=False, mask=None, mode="train"):
        ...
        for b in range(bsz):
            ...
            if self.config.use_wider_ent_decode and not is_gt and mode != "train" and False:
