...
def main(func_mode=False, **kwargs):
    ...
    model_checkpoint = pl.callbacks.ModelCheckpoint( # type: ignore
        monitor="val_f1", mode="max", save_last=False, save_top_k=1,
    ...
    if config.test_from_ckpt:
        ...
        elif isinstance(config.test_opt1, int):
            for _p in os.listdir(os.path.join(config.output_dir, "checkpoints")):
                if f"epoch={config.test_opt1}" in _p:
                    ckpt_path = os.path.join(config.output_dir, "checkpoints", _p)
                    print(cp.blue("Found ckpt: "), ckpt_path)
                    break
