def f1_score(outputs, pred_name, gold_name, slice=None):
    ...
    for val_out in outputs:
        ...
        # if len(set(gold_triples)) != len(gold_triples):
        #     print(f"Gold Set({len(set(gold_triples))}) != Len({len(gold_triples)})")

        # if len(set(pred_triples)) != len(pred_triples):
        #     print(f"Pred Set({len(set(pred_triples))}) != Len({len(pred_triples)})")
